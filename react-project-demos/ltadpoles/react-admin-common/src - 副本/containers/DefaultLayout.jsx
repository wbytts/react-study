...
class DefaultLayout extends Component {
  ...
  render() {
    ...
    const routeRender = item => props => {
      if (auth) {
        if (item.auth && item.auth.indexOf(auth) !== -1) {
      ...
      <Layout className="app">
        ...
        <Layout style={{ marginLeft: menuToggle ? '80px' : '200px', minHeight: '100vh' }}>
          ...
          <Content className="content">
            <Switch>
              {routes.map(item => {
                return <Route key={item.path} path={item.path} exact={item.exact} render={routeRender(item)}></Route>;
              })}
              <Redirect to="/404" />
            </Switch>
          </Content>
